...
// Load Instagram Posts reais
async function loadPosts() {
  try {
    const username = "defesacivil_co" // usuário Instagram
      ...
      slidesHTML += `
        <article class="news-card">
          ...
          <div class="news-content">
            ...
            <a href="${post.external_link}" class="news-link" target="_blank">
              Ver no Instagram
...
async function loadQuickLinks() {
  try {
    const response = await fetch("/quick-links.json")
    const data = await response.json()
    ...
    container.innerHTML = uniqueLinks.map((link, index) => `
...
async function loadAds() {
  try {
    ...
    if (leftContainer && leftAds.length > 0) {
      leftContainer.innerHTML = leftAds.slice(0, 3).map(ad => `
    ...
    if (rightContainer && leftAds.length > 3) {
      rightContainer.innerHTML = leftAds.slice(3, 6).map(ad => `
        <a href="${ad.link_url || "#"}" class="sidebar-ad ${ad.size}" target="_blank">
          <img src="${ad.image_url}" alt="${ad.title || "Propaganda"}">
        </a>
      `).join("")
    ...
    if (betweenContainer && betweenAds.length > 0) {
      betweenContainer.innerHTML = betweenAds.map(ad => `
...
async function checkHiddenElements() {
  try {
    const response = await fetch("/hidden-elements.json")
    const data = await response.json()

    data.forEach(element => {
      const el = document.getElementById(element.element_key)
      if (el && element.is_hidden) el.style.display = "none"
    })
  } catch (error) {
